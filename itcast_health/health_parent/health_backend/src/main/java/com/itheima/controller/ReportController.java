...
public class ReportController {
    ...
    public Result getMemberReport() {
        ...
        Calendar c = Calendar.getInstance();//获取日历对象.模拟时间就是当前时间
        c.add(Calendar.MONTH, -12);//获得当前时间往前推12个月的时间
        for (int i = 0; i < 12; i++) {
            c.add(Calendar.MONTH, 1);//获得当前时间忘后退一个月
            Date time = c.getTime();
    ...
    public Result getBusinessReportData() {
        ...
        try {
            Report data = reportService.getBusinessReportData();
    ...
    public Result exportBusinessReport(HttpServletRequest request, HttpServletResponse response)   {
        try {
            ...
            int rowNum = 12;
            List<HotSetmeal> hotSetmeal = report.getHotSetmeal();
            for (HotSetmeal hot : hotSetmeal) {
                String name = hot.getName();
                Integer setmeal_count = hot.getSetmeal_count();
                float proportion = hot.getProportion();
